...
import 'package:here4help/chat/widgets/support_dynamic_action_bar.dart';
...
import 'package:here4help/chat/services/support_chat_service.dart'
    as SupportChatService;
...
class SupportChatDetailPage extends StatefulWidget {
  const SupportChatDetailPage({super.key, this.data});
...
class _SupportChatDetailPageState extends State<SupportChatDetailPage>
  ...
  int? _getOpponentUserId() {
    ...
    try {
      ...
      final userId = room['user_id'] ?? room['userId'];
      final adminId = room['admin_id'] ?? room['adminId'];

      debugPrint('  - userId (原始): $userId (類型: ${userId.runtimeType})');
      debugPrint('  - adminId (原始): $adminId (類型: ${adminId.runtimeType})');
      ...
      final int? user = (userId is int) ? userId : int.tryParse('$userId');
      final int? admin = (adminId is int) ? adminId : int.tryParse('$adminId');

      debugPrint('  - user (解析後): $user');
      debugPrint('  - admin (解析後): $admin');
      debugPrint('  - currentUserId: $_currentUserId');

      if (user == _currentUserId) {
        debugPrint('✅ [ChatDetailPage] 當前用戶是 user，返回 admin: $admin');
        return admin;
      }
      if (admin == _currentUserId) {
        debugPrint('✅ [ChatDetailPage] 當前用戶是 admin，返回 user: $user');
        return user;
      }

      // 如果都不匹配，返回 admin 或 user
      final result = admin ?? user;
  ...
  (String, String?, double, int) _getParticipantInfo() {
    try {
      ...
      // 取得 admin_id
      final adminId = room['admin_id'];
      final int? adminIdInt =
          (adminId is int) ? adminId : int.tryParse('$adminId');

      debugPrint('🔍 [_getParticipantInfo] admin_id: $adminIdInt');

      // 判斷當前用戶角色來決定資料來源
      if (_userRole == 'user') {
        // 如果我是客戶，admin 就是對方，使用 chat_partner_info
        final chatPartnerInfo = _chatPartnerInfo;
        if (chatPartnerInfo != null) {
          final name =
              chatPartnerInfo['name']?.toString().trim() ?? 'Applicant';
          ...
          debugPrint(
              '✅ [_getParticipantInfo] 客戶視角 - 從 chat_partner_info 獲取管理員資料');
          debugPrint('  - 姓名: $name, 評分: $avgRating ($count 評論)');

          return (name, avatar?.toString(), avgRating, count);
        }
      } else {
        // 如果我是管理員，admin 就是我自己，需要從其他地方獲取我的資料
        // 這種情況下應該不會顯示 Resume Dialog，但為了完整性還是處理
        debugPrint('⚠️ [_getParticipantInfo] 管理員視角 - 不應該顯示自己的 Resume');

        // 從 room 資料中取得 admin 資訊
        final adminName = room['admin_name']?.toString().trim() ??
            '${room['admin_name']?.toString().trim()}(You)';
        final adminAvatar = room['admin_avatar']?.toString();

        // 從 RatingService 獲取評分（如果需要的話）
        return (adminName, adminAvatar, 0.0, 0);
      }
  ...
  Future<void> _refreshTaskStatusOnExpand() async {
    ...
    try {
      ...
      // 重新獲取聊天室數據以更新任務狀態
      final rawChatData = await SupportChatService.ChatService()
  ...
  Future<void> _sendImageMessage(ImageResult image,
    ...
    try {
      ...
      // 上傳圖片
      final uploadResult =
          await SupportChatService.ChatService().uploadAttachment(
  ...
  @override
  void dispose() {
    // 最後一次保險：離開時嘗試標記到目前列表最後一則（避免重複呼叫）
    try {
      if (_currentRoomId != null &&
        ...
        if (lastId > 0 && lastId > (_myLastReadMessageId ?? 0)) {
          NotificationCenter()
              .service
  ...
  Future<void> _initializeChat() async {
    try {
      ...
      if (mounted) {
        setState(() {
          _hasError = true;
          _errorMessage = 'Load support chat detail failed: $e';
        });
      }
    }
  }
  ...
  Future<void> _loadChatMessagesFromDatabase() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        // 標記已讀（讀到列表中的最後一則訊息）- 避免重複呼叫
        // Socket 訊息更新時不需要重複標記已讀，只在用戶主動操作時標記
  ...
  Future<void> _sendMessage({String? textOverride}) async {
    ...
    try {
      ...
      // 客服聊天室不需要 taskId
      final taskId = _isSupportRoom ? null : (_task?['id']?.toString() ?? '');
      if (!_isSupportRoom && (taskId == null || taskId.isEmpty)) {
      ...
      try {
        if (_socketService.isConnected) {
          ...
          debugPrint('✅ Socket 訊息已發送');
        } else {
          debugPrint('⚠️ Socket 未連接，僅使用 HTTP API');
        }
      } catch (e) {
        debugPrint('❌ Socket 發送失敗: $e');
      }

      // 發送到後端
      final result = await SupportChatService.ChatService().sendMessage(
        roomId: _currentRoomId!,
        message: text,
        taskId: taskId,
      );
  ...
  Future<void> _handleWithdrawApplication() async {
    ...
    try {
      ...
      if (confirmed == true) {
        await SupportChatService.ChatService()
            .withdrawApplication(taskId: taskId);
        if (mounted) {
          // ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _openReportSheet() async {
    ...
    try {
      // 先檢查用戶是否已經檢舉過
      final reportStatus =
          await SupportChatService.ChatService().checkReportStatus(
        roomId: _currentRoomId!,
      );
