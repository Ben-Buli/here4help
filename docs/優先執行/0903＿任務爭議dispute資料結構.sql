...
CREATE TABLE `chat_messages` (
  ...
  `kind` enum('text','image','file','system','resume') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT 'text',
...
--
-- 資料表結構 `fee_revenue_ledger`
--

CREATE TABLE `fee_revenue_ledger` (
  `id` bigint UNSIGNED NOT NULL,
  `fee_type` enum('task_completion') COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '目前只記錄任務完成手續費',
  `src_transaction_id` bigint UNSIGNED DEFAULT NULL COMMENT '對應 point_transactions.id（產生此手續費的主交易）',
  `task_id` bigint UNSIGNED DEFAULT NULL,
  `payer_user_id` bigint UNSIGNED DEFAULT NULL COMMENT '被收取手續費者（通常任務發佈者）',
  `amount_points` int UNSIGNED NOT NULL COMMENT '實際收取的手續費（點數，已整數四捨五入後）',
  `rate` decimal(5,4) NOT NULL COMMENT '當下生效費率（例如 0.025 = 2.5%）',
...
--
-- 資料表結構 `official_bank_accounts`
--

CREATE TABLE `official_bank_accounts` (
  `id` int NOT NULL,
  `bank_name` varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '銀行名稱',
  `account_number` varchar(50) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '帳號（純數字）',
  `account_holder` varchar(100) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '戶名',
  `is_active` tinyint(1) DEFAULT '0' COMMENT '是否啟用（僅一個可為1）',
  `admin_id` int DEFAULT NULL COMMENT '負責管理員ID',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `point_deposit_requests` (
  `id` bigint UNSIGNED NOT NULL,
  `user_id` bigint UNSIGNED NOT NULL COMMENT '申請的使用者 (對應 users.id)',
  `amount_points` int UNSIGNED NOT NULL DEFAULT '0' COMMENT '申請儲值的點數',
  `bank_account_last5` char(5) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '匯款帳號末五碼',
  `status` enum('pending','approved','rejected') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'pending',
  `approver_reply_description` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '管理員審核回覆',
  `approver_id` int DEFAULT NULL COMMENT '審核管理員 (對應 admins.id)',
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE `point_transactions` (
  `id` bigint NOT NULL,
  `user_id` bigint UNSIGNED NOT NULL,
  `transaction_type` enum('earn','spend','deposit','fee','refund','adjustment') COLLATE utf8mb4_unicode_ci NOT NULL,
  `amount` int NOT NULL COMMENT '金額（正數收入，負數支出）',
  `description` varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL,
  `related_task_id` varchar(36) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
...
CREATE TABLE `support_chat_rooms` (
  ...
  `dispute_id` bigint UNSIGNED DEFAULT NULL,
  `creator_id` bigint UNSIGNED NOT NULL,
  `participant_id` bigint UNSIGNED DEFAULT NULL COMMENT 'Null代表尚未有Admin Claim接手此客服聊天室對應的客服事件',
  `type` enum('application','task','support','dispute') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'task',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `support_events` (
  ...
  `description` text COLLATE utf8mb4_unicode_ci,
  `status` enum('submitted','in_progress','resolved') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'submitted',
...
CREATE TABLE `support_event_logs` (
  ...
  `old_status` enum('submitted','in_progress','resolved') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `new_status` enum('submitted','in_progress','resolved') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL,
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `tasks` (
  ...
  `language_requirement` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT 'English',
...
CREATE TABLE `task_applications` (
  ...
  `user_id` bigint UNSIGNED NOT NULL,
  `status` enum('applied','accepted','pending','dispute','completed','rejected','cancelled','withdrawn') CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'applied' COMMENT '	•	applied — 已投遞 → task: open\r\n	•	accepted — 已被指派 → task: in_progress\r\n	•	rejected — 已被拒絕（任務發布者拒絕此應徵） → task: open（任務仍在招募中）\r\n	•	pending — 待處理 / 待確認（申請並正在等待發布者審核任務完成） → task: pending cofirmation\r\n	•	completed — 已完成（應徵的任務完成並確認結束） → task: completed\r\n	•	cancelled — 已取消（任務取消，或應徵因外部原因被取消） → task: cancelled\r\n	•	dispute — 爭議中（任務執行中產生爭議，等待仲裁處理） → task: dispute\r\n	•	withdrawn — 使用者已撤銷（應徵者主動撤回申請，或因停用/刪帳自動撤回） → task: open（回到待選狀態，其他人仍可應徵）',
...
--
-- 資料表結構 `task_completion_points_fee_settings`
--

CREATE TABLE `task_completion_points_fee_settings` (
  `id` bigint UNSIGNED NOT NULL,
  `rate` decimal(5,4) NOT NULL COMMENT '百分比，0.02 = 2%',
  `description` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
...
--
-- 觸發器 `task_completion_points_fee_settings`
--
DELIMITER $$
CREATE TRIGGER `tcpf_before_insert` BEFORE INSERT ON `task_completion_points_fee_settings` FOR EACH ROW BEGIN
  -- 確保新資料至少有 is_active 值
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `tcpf_before_update` BEFORE UPDATE ON `task_completion_points_fee_settings` FOR EACH ROW BEGIN
  IF NEW.is_active = 1 AND (OLD.is_active <> 1) THEN
    UPDATE task_completion_points_fee_settings
      SET is_active = 0
...
CREATE TABLE `task_dispute_events` (
  ...
  `description` text COLLATE utf8mb4_unicode_ci COMMENT '爭議描述 (用戶提出的)',
  ...
  `rating` tinyint DEFAULT NULL COMMENT '事後評分',
  `review` text COLLATE utf8mb4_unicode_ci COMMENT '事後評論',
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
...
--
-- 資料表結構 `task_dispute_event_logs`
--

CREATE TABLE `task_dispute_event_logs` (
  `id` bigint UNSIGNED NOT NULL,
  `event_id` bigint UNSIGNED NOT NULL COMMENT '對應的爭議事件',
  `admin_id` int DEFAULT NULL COMMENT '操作人 (管理員)',
  `old_status` enum('submitted','in_progress','resolved') COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '舊的流程狀態',
  `new_status` enum('submitted','in_progress','resolved') COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '新的流程狀態',
  `old_decision` enum('completed','back_to_progress','reset') COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '舊的裁決',
  `new_decision` enum('completed','back_to_progress','reset') COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '新的裁決',
...
CREATE TABLE `task_reports` (
  ...
  `task_id` varchar(36) COLLATE utf8mb4_unicode_ci NOT NULL,
  `reporter_id` bigint UNSIGNED NOT NULL,
  `reason` enum('spam_advertising','fraud_scam','misleading_false_info','illegal_activity','abusive_offensive_content','duplicate_repeated_posting','unreasonable_reward_conditions','other') COLLATE utf8mb4_unicode_ci NOT NULL,
  `description` text COLLATE utf8mb4_unicode_ci NOT NULL,
...
CREATE TABLE `users` (
  ...
  `about_me` text COLLATE utf8mb4_unicode_ci COMMENT '使用者自我介紹',
  ...
  `permission` int DEFAULT '0' COMMENT 'ALTER TABLE `users`\r\n  MODIFY `permission` int DEFAULT 0\r\n  COMMENT ''unverified新用戶未認證:0, verified已認證用戶:1, admin管理員:99, admin_suspended被管理員停權:-1, admin_soft_deleted被管理員軟刪除:-2, self_suspended被管理員軟刪除:-3, self_deleted用戶自行軟刪除:-4'';',
...
CREATE TABLE `user_active_log` (
  `id` bigint UNSIGNED NOT NULL,
  `user_id` bigint UNSIGNED NOT NULL,
  `actor_type` varchar(16) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'user|admin|system',
  `actor_id` bigint UNSIGNED DEFAULT NULL COMMENT '對應 users.id 或 admins.id；system 可為 NULL',
  `action` varchar(64) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT 'register|verify_completed|permission_change|status_change|suspend|ban|deactivate|reactivate|soft_delete|...任意字串',
  `field` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT 'permission 或 status；無實際欄位變更可為 NULL',
  `old_value` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `new_value` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `reason` text COLLATE utf8mb4_unicode_ci,
...
--
-- 資料表索引 `fee_revenue_ledger`
--
ALTER TABLE `fee_revenue_ledger`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_type_created` (`fee_type`,`created_at`),
  ADD KEY `idx_task` (`task_id`),
  ADD KEY `idx_src_tx` (`src_transaction_id`),
  ADD KEY `idx_payer` (`payer_user_id`);
...
--
-- 資料表索引 `official_bank_accounts`
--
ALTER TABLE `official_bank_accounts`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `uk_active_account` (`is_active`);
...
ALTER TABLE `point_deposit_requests`
  ...
  ADD KEY `idx_user_last5_amount_pending` (`user_id`,`bank_account_last5`,`amount_points`,`status`);
...
ALTER TABLE `point_transactions`
  ...
  ADD KEY `idx_type_created` (`transaction_type`,`created_at`);
...
--
-- 資料表索引 `support_chat_reads`
--
ALTER TABLE `support_chat_reads`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `uq_support_reads_user_room` (`user_id`,`room_id`),
  ADD KEY `idx_support_reads_user` (`user_id`),
  ADD KEY `idx_support_reads_room` (`room_id`);

--
-- 資料表索引 `support_chat_rooms`
--
ALTER TABLE `support_chat_rooms`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `uniq_support_task_creator_participant` (`task_id`,`creator_id`,`participant_id`),
  ADD KEY `idx_support_room_creator` (`creator_id`),
  ADD KEY `idx_support_room_participant` (`participant_id`),
  ADD KEY `idx_support_room_dispute` (`dispute_id`);
...
ALTER TABLE `support_events`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_chat_room_status` (`support_chat_room_id`,`status`),
  ADD KEY `idx_user_id` (`user_id`),
...
--
-- 資料表索引 `task_completion_points_fee_settings`
--
ALTER TABLE `task_completion_points_fee_settings`
  ADD PRIMARY KEY (`id`);
...
--
-- 資料表索引 `task_dispute_events`
--
ALTER TABLE `task_dispute_events`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_task` (`task_id`),
  ADD KEY `idx_chat_room` (`task_dispute_chat_room_id`),
  ADD KEY `idx_user` (`user_id`),
  ADD KEY `idx_admin` (`admin_id`);

--
-- 資料表索引 `task_dispute_event_logs`
--
ALTER TABLE `task_dispute_event_logs`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_event_id` (`event_id`),
  ADD KEY `fk_dispute_logs_admin` (`admin_id`);
...
ALTER TABLE `user_active_log`
  ...
  ADD KEY `idx_actor` (`actor_type`,`actor_id`,`created_at`),
...
--
-- 使用資料表自動遞增(AUTO_INCREMENT) `chat_reports`
--
ALTER TABLE `chat_reports`
  MODIFY `id` bigint UNSIGNED NOT NULL AUTO_INCREMENT;
...
--
-- 使用資料表自動遞增(AUTO_INCREMENT) `fee_revenue_ledger`
--
ALTER TABLE `fee_revenue_ledger`
  MODIFY `id` bigint UNSIGNED NOT NULL AUTO_INCREMENT;
...
--
-- 使用資料表自動遞增(AUTO_INCREMENT) `official_bank_accounts`
--
ALTER TABLE `official_bank_accounts`
  MODIFY `id` int NOT NULL AUTO_INCREMENT;
...
--
-- 使用資料表自動遞增(AUTO_INCREMENT) `support_chat_messages`
--
ALTER TABLE `support_chat_messages`
  MODIFY `id` bigint NOT NULL AUTO_INCREMENT;

--
-- 使用資料表自動遞增(AUTO_INCREMENT) `support_chat_reads`
--
ALTER TABLE `support_chat_reads`
  MODIFY `id` bigint NOT NULL AUTO_INCREMENT;

--
-- 使用資料表自動遞增(AUTO_INCREMENT) `support_chat_rooms`
--
ALTER TABLE `support_chat_rooms`
  MODIFY `id` bigint NOT NULL AUTO_INCREMENT;
...
--
-- 使用資料表自動遞增(AUTO_INCREMENT) `task_completion_points_fee_settings`
--
ALTER TABLE `task_completion_points_fee_settings`
  MODIFY `id` bigint UNSIGNED NOT NULL AUTO_INCREMENT;
...
--
-- 使用資料表自動遞增(AUTO_INCREMENT) `task_dispute_events`
--
ALTER TABLE `task_dispute_events`
  MODIFY `id` bigint UNSIGNED NOT NULL AUTO_INCREMENT;

--
-- 使用資料表自動遞增(AUTO_INCREMENT) `task_dispute_event_logs`
--
ALTER TABLE `task_dispute_event_logs`
  MODIFY `id` bigint UNSIGNED NOT NULL AUTO_INCREMENT;
...
--
ALTER TABLE `point_deposit_requests`
  ADD CONSTRAINT `fk_pdr_admin` FOREIGN KEY (`approver_id`) REFERENCES `admins` (`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
--
-- 資料表的限制式 `support_chat_messages`
--
ALTER TABLE `support_chat_messages`
  ADD CONSTRAINT `fk_support_messages_room` FOREIGN KEY (`room_id`) REFERENCES `support_chat_rooms` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `fk_support_messages_user` FOREIGN KEY (`from_user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE;

--
-- 資料表的限制式 `support_chat_reads`
--
ALTER TABLE `support_chat_reads`
  ADD CONSTRAINT `fk_support_reads_room` FOREIGN KEY (`room_id`) REFERENCES `support_chat_rooms` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `fk_support_reads_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE;

--
-- 資料表的限制式 `support_chat_rooms`
--
ALTER TABLE `support_chat_rooms`
  ADD CONSTRAINT `fk_support_room_creator` FOREIGN KEY (`creator_id`) REFERENCES `users` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `fk_support_room_dispute` FOREIGN KEY (`dispute_id`) REFERENCES `task_disputes` (`id`) ON DELETE SET NULL,
  ADD CONSTRAINT `fk_support_room_participant` FOREIGN KEY (`participant_id`) REFERENCES `users` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `fk_support_room_task` FOREIGN KEY (`task_id`) REFERENCES `tasks` (`id`) ON DELETE CASCADE;
...
ALTER TABLE `support_events`
  ...
  ADD CONSTRAINT `fk_support_events_support_chat_room` FOREIGN KEY (`support_chat_room_id`) REFERENCES `support_chat_rooms` (`id`) ON DELETE CASCADE,
...
--
-- 資料表的限制式 `task_dispute_events`
--
ALTER TABLE `task_dispute_events`
  ADD CONSTRAINT `fk_task_dispute_events_admin` FOREIGN KEY (`admin_id`) REFERENCES `admins` (`id`) ON DELETE SET NULL,
  ADD CONSTRAINT `fk_task_dispute_events_chat_room` FOREIGN KEY (`task_dispute_chat_room_id`) REFERENCES `chat_rooms` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `fk_task_dispute_events_task` FOREIGN KEY (`task_id`) REFERENCES `tasks` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `fk_task_dispute_events_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE;

--
-- 資料表的限制式 `task_dispute_event_logs`
--
ALTER TABLE `task_dispute_event_logs`
  ADD CONSTRAINT `fk_dispute_logs_admin` FOREIGN KEY (`admin_id`) REFERENCES `admins` (`id`) ON DELETE SET NULL,
  ADD CONSTRAINT `fk_dispute_logs_event` FOREIGN KEY (`event_id`) REFERENCES `task_dispute_events` (`id`) ON DELETE CASCADE;
