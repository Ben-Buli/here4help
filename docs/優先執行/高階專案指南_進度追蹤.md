...
### 順序6：帳號安全與保護 📋
**目標**：實作用戶帳號安全機制與風險檢查
- [ ] T1-BE-SECURITY-GUARD：GET /account/risky-actions-check, POST /account/deactivate, POST /account/reactivate
- [ ] T2-FLT-SECURITY-UI：/account/security UI 與保護流程
- **驗收**：有進行中/發布中任務時先提示，並且禁止在有尚未完成任務時停用帳戶；自停用後可再啟用（非被管理員停權）
