...
- 全程式碼搜尋並移除：`users.google_id`、`users.provider` 的讀寫。
- 顯示「綁定的登入方式」：改讀 `user_identities` 彙整（例如 `GROUP_CONCAT(provider)`）。

---

## 6) 測試案例（關鍵路徑）

1. **密碼註冊／登入**：註冊→驗證信→登入；錯誤密碼節流。
2. **第三方新用戶**：Google 登入→建立 users+identity→登入；`email_verified_at` 規則驗證。
3. **第三方與既有帳號綁定**：既有 Email/密碼帳號→ Google 登入→撞名→要求綁定→完成後以 Google 直接登入。
4. **多方綁定**：同帳號綁 Google + Apple；`user_identities` 兩筆資料。
5. **郵件流程**：重寄驗證、重設密碼 token 有效/過期，HTTPS 連結正確。
6. **清理後驗證**：DROP `users.provider/google_id` 後所有 API 仍正常。
