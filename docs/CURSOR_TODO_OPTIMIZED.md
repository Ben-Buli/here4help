...
**當前狀態**: 29.2% 完成 (19/65 任務) | **目標**: 10天內完成 100% (8/8-8/17) | **完成機率**: 85%

### 🎯 核心指標
- **Flutter App**: 19/49 (38.8%) 🔄
- **後台管理**: 0/4 (0.0%) 📋
- **cPanel 部署**: 0/5 (0.0%) 📋
- **第三方登入**: 0/7 (0.0%) 📋
...
|------|----------|----------|------|------|--------|
| **Day 1 (8/8)** | 聊天室功能 | 20,21,26 | ⭐⭐⭐ | 8-10h | 35.4% |
| **Day 2 (8/9)** | 聊天室收尾 + 未讀 + 遷移執行 | 20,21(HTTP),26,27(部分) | ⭐⭐⭐⭐ | 10-12h | 41.5% |
...
- 狀態: 部分完成（8/9 收尾：未讀通知、Posted/My Works 真實資料、資訊面板最終版）
...
**26. [ ] 未讀訊息通知系統**
- 檔案: `lib/services/notification_service.dart` (新增)
- 操作: 全局通知服務, bottom navbar 標記
- 狀態: 待執行

#### 🗓️ Day 2 - 8/9 實作計畫（取代原 Day 2）
**A. 20 - 聊天室列表收尾（4h）**
- 未讀數聚合與顯示（Navbar badge + 房間內 badge）
- Posted/My Works 改為真實資料源（`applications/list_by_user.php` + `acceptor_id`）
- 任務資訊 Bottom Sheet 交互與按鈕位置定稿

**B. 21 - 聊天室詳情（HTTP 先行 3h）**
- 建立 `chat/rooms/open_or_get.php`、`chat/messages/send.php` 端點
- /chat/detail 顯示 system message（`application_resume`） + 歷史訊息載入 + 送出訊息（HTTP）
- Socket.IO 架構與事件規劃（文檔）
...
**D. 27（部分） - 遷移執行（2h）**
- 執行 `creator_id` 回填與 `creator_name` 刪欄
- 移除前端殘留 `creator_name` 寫入，全面使用 JOIN 的 `creator_name`
- 清理殘留 `task['status']` 舊字串引用，全面用 `status_code/status_display`
