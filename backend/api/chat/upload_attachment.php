...
try {
  ...
  // JWT 認證 - 使用統一的 validateRequest 方法（支援 header 和 query/POST token）
  $auth = JWTManager::validateRequest();
  
  if (!$auth['valid']) {
    error_log("❌ [upload_attachment.php] JWT 驗證失敗: " . ($auth['message'] ?? 'Unauthorized'));
    Response::error($auth['message'] ?? 'Unauthorized', 401);
  }
  
  $user_id = (int)$auth['payload']['user_id'];
  error_log("✅ [upload_attachment.php] JWT 驗證成功，用戶ID: $user_id");
