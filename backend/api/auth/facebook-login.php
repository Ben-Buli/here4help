...
try {
    ...
    if ($existingIdentity) {
        ...
        error_log("Facebook Login - 新用戶，檢查 email 是否已存在...");
        ...
        if (!empty($email)) {
            ...
            if ($existingUser) {
                error_log("Facebook Login - Email 已存在，需要綁定到現有帳號，用戶 ID: {$existingUser['id']}");
                ...
                error_log("Facebook Login - 完全新用戶，建立新帳號...");
                ...
                error_log("Facebook Login - 新用戶建立成功，用戶 ID: $userId");
                ...
                error_log("Facebook Login - 新用戶和 user_identity 建立完成");
            }
        } else {
            error_log("Facebook Login - 無 email 的新用戶，建立新帳號...");
            ...
            $userId = $db->lastInsertId();
            error_log("Facebook Login - 無 email 新用戶建立成功，用戶 ID: $userId");
            ...
            error_log("Facebook Login - 無 email 新用戶和 user_identity 建立完成");
        }
    }
    ...
    error_log("Facebook Login - 登入成功，用戶 ID: {$user['id']}, 新用戶: " . ($isNewUser ? '是' : '否'));
