...
try {
    ...
    $tasksQuery = "
        SELECT 
            ...
            ROUND(AVG(tr.rating)) AS avg_rating_for_creator,
            EXISTS(
                SELECT 1 FROM task_ratings tr2 
                WHERE tr2.task_id = t.id AND tr2.rater_id = ta.user_id
            ) AS participant_already_rated,
            (
                SELECT JSON_OBJECT(
                    'rating', tr3.rating,
                    'comment', tr3.comment,
        ...
        WHERE ta.user_id = ?
        GROUP BY t.id, t.title, t.task_date, t.reward_point, t.status_id, ts.display_name, t.creator_id, ta.id, u.name, u.avatar_url
    ...
    $formattedTasks = array_map(function($task) use ($userId) {
        ...
        return [
            ...
            'avg_rating_for_creator' => $task['avg_rating_for_creator'] ? (int)$task['avg_rating_for_creator'] : null,
            'has_rating_from_creator' => $ratingData !== null,
            'creator_rating' => $ratingData,
            'can_rate' => (int)$task['status_id'] === 5 && !$task['participant_already_rated']
        ];
    }, $tasks);
    ...
    ], 'Accepted tasks retrieved successfully');
    
} catch (Exception $e) {
    error_log("Accepted tasks error: " . $e->getMessage());
    Response::error('Failed to retrieve accepted tasks: ' . $e->getMessage(), 500);
}
?>
