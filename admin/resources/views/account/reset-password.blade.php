...
<head>
    ...
    <title>Reset Password | Here4Help</title>
...
<body>
    <main class="card">
        ...
        <p class="description">Enter a new password for your Here4Help account. This secure link expires one hour after it was issued.</p>
        ...
        <form id="resetForm" novalidate>
            ...
            <ul class="requirements" id="requirements">
                <li data-rule="length">Password must be at least 6 characters</li>
                <li data-rule="allowed">Only letters and numbers (A-Z, a-z, 0-9)</li>
            </ul>

            <div class="strength-indicator weak" id="strengthBox">
                <div class="strength-status weak" id="strengthStatus">Password strength：<span id="strengthLabel" class="strength-value weak">Weak</span></div>
    ...
    <script>
        (function () {
            ...
            if (!token || !email) {
                setStatus('This link is invalid. Please request a new password reset link from support.')
                form.querySelectorAll('input, button').forEach((el) => el.setAttribute('disabled', 'disabled'))
                return
            }

            emailBadge.textContent = `Resetting password for ${email}`
            ...
            const updateStrengthIndicator = () => {
                ...
                const variety = [/[A-Z]/, /[a-z]/, /\d/].reduce((acc, regex) => acc + (regex.test(value) ? 1 : 0), 0)
                if (variety >= 2) score++
                if (variety === 3) score++
            ...
            form.addEventListener('submit', async (event) => {
                ...
                try {
                    const response = await fetch('/backend/api/account/reset-password.php', {
                        method: 'POST',
                    ...
                    if (!response.ok || !result.success) {
                        throw new Error(result.message || 'Unable to reset password. Please try again.')
                    }

                    setStatus('Password reset successfully! You can now return to the app and log in with your new password.', 'success')
                    form.querySelectorAll('input, button').forEach((el) => el.setAttribute('disabled', 'disabled'))
                } catch (error) {
